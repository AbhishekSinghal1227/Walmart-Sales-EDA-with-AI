...
def general_question_tool(user_input: str) -> str:
    """
    Step 8: Execute AI-generated Python code and provide actual output using df
    """
    columns = list(df.columns)
    llm_response = llm.invoke(f"""
    ...
    1. Python (pandas) code to answer the question for df where df is Walmart.csv
       ⚠️ Important: Include calculation of Sales if not present and assign final result to _result
    2. SQL query (assume table name = walmart)
    ...
    # Prepend Sales calculation if missing
    sales_calc = """
